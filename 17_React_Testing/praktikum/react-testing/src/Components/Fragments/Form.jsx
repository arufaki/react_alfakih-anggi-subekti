...
export default function Form({ onSubmit, initialData }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (!productName || productName.length > 25) {
            ...
            alert("Image Size too big max 2MB");
            return;
        } else if (!imageValidation.includes(imageFile.type)) {
            alert("No Valid image Type must be JPEG/JPG/PNG");
            return;
        ...
        <section className="flex flex-wrap items-center justify-center flex-col">
            <form className="group w-[600px]" onSubmit={handleSubmit}>
                ...
                <div className="product-name test mb-9 w-[300px]">
                    ...
                    />
                    {error.name && (
                        <p className="mt-2 font-roboto text-sm text-red-500" role="alert-name">
                            {error.name}
                        </p>
                    )}
                </div>
                <div className="product-category test mb-9 w-[224px]">
                    ...
                    <Select name="category" value={values.category} onChange={handleChange} data-testid="select-category" />
                </div>
                <div className="product-image test mb-9 w-[202px]">
                    ...
                    />
                    {error.image && <p className="mt-2 font-roboto text-sm text-red-500">{error.image}</p>}
                </div>
                ...
                <div className="product-description test mb-8">
                    ...
                    />
                    {error.description && (
                        <p className="mt-2 font-roboto text-sm text-red-500" role="alert">
                            {error.description}
                        </p>
                    )}
                </div>
                <div className="product-price test mb-9">
                    ...
                    />
                    {error.price && (
                        <p className="mt-2 font-roboto text-sm text-red-500" data-testid="alert">
                            {error.price}
                        </p>
                    )}
                </div>
                <Button type="submit" label="Submit" variant="submit" data-testid="submit-button" />
                {errorSubmit && <div style={{ color: "red" }}>{errorSubmit}</div>}
            </form>

            <p data-testid="product-display">Product Name: {values.name}</p>
        </section>
    );
}
