const Sidebar = ({ onClick, datas, handleClickHistory, response }) => {
    ...
    return (
        <div className="h-screen w-60 bg-slate-500">
            <button className={`mt-2 ml-3 p-3 rounded-lg cursor-pointer ${isResponsesEmpty ? "bg-gray-300" : "bg-blue-300"}`} onClick={isResponsesEmpty ? null : onClick} disabled={isResponsesEmpty}>
                New Chat
            </button>
            <div className="mt-5">
                {historyData
